...
function overview(data, el) {
    let svg = select(el).html(null).append("svg");
    let g = svg.append("g");
    ...
    function draw() {
        ...
        dem.attr("d", arc$$1);
        dem_error.attr("d", arc$$1);
        dem_gain.attr("d", arc$$1);
        gop.attr("d", arc$$1);
        gop_error.attr("d", arc$$1);
...
function history(data, el) {
    ...
    let maxOdds = 1.05 * max(data, d => Math.abs(d.odds));
    ...
    function draw(max$$1 = electionDay) {
        w = el.clientWidth; 
        h = 320;
        ...
        svg.select(".dem.line").attr("d", line$$1);

        line$$1.y(d => y(-d.odds));
        svg.select(".gop.line").attr("d", line$$1);

        resetTooltip();
    }

    function resetTooltip(max$$1 = electionDay) {
        box.attr("transform", `translate(${w} 0)`);
        shade.attr("width", margin.R);
        dateText.text(dateFormat(max$$1));
...
function generic(data, el) {
    ...
    function draw() {
        ...
        let line$$1 = line()
            .x(d => x(d.date))
            .y(d => y(d.margin));
        svg.select(".past.line").attr("d", line$$1);
        svg.select(".future.line").attr("d", line$$1);
...
function outcomes(data, el) {
    let table = select(el).append("tbody");
